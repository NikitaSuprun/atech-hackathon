...
void ST7735_TFT::begin() {
    // Hardware SPI: the ESP32-S3 GPIO matrix routes the SPI peripheral to any
    // pins, so we get real 40MHz clocking — ~100x faster than bit-banged software
    // SPI for the 25KB full-frame canvas push (the difference between a ~12fps and
    // a smooth dashboard). The panel is write-only, so no MISO.
    SPI.begin(_sclkPin, -1, _mosiPin, _csPin);
    _tft = new Adafruit_ST7735(&SPI, _csPin, _dcPin, -1);

    _initDisplay();  // panel init (BGR MADCTL, 40MHz SPI) + fill black
    _drawSplash();   // Atech logo; stays until the first canvas->display() paints over it
...
void ST7735_TFT::textBounds(const char* s, int16_t* x1, int16_t* y1, uint16_t* w, uint16_t* h) {
    _gfx()->getTextBounds(s, (int16_t)0, (int16_t)0, x1, y1, w, h);
}
