...
// drives the speaker, and streams the composed Color[108] frames over the COBS
// serial link to the screen board. Pins match the Atech 14-port controller build.
#include <Arduino.h>
...
static ConsoleBrain  brain(knob0, knob1, speaker);
...
void setup() {
    ...
    knob1.begin();
    speaker.begin();
    speaker.setVolume(0.2f);          // MAX98357A clips near 1.0 — start gentle
